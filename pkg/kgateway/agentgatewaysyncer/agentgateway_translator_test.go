...
func TestBasic(t *testing.T) {
	...
	t.Run("Backend policy with mcp authn remote and unspecified idp", func(t *testing.T) {
		test(t, translatorTestCase{
			inputFile:  "backend/no-idp-mcp-authn.yaml",
			outputFile: "backend/no-idp-mcp-authn.yaml",
			gwNN: types.NamespacedName{
				Namespace: "default",
				Name:      "example-gateway",
			},

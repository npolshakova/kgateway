...
The proposal is to implement two new fields for JWT Authentication and Authorization on the existing TrafficPolicy.
...
Alternatively, the rbac rules can be defined closer to the Istio-style and expanded eventually beyond JWT:
```yaml
rbac:
  action: Allow
  rules:

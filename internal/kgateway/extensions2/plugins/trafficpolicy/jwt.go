...
// jwtForPolicy translates the jwt spec into an envoy jwt policy and stores it in the traffic policy IR
func (b *TrafficPolicyBuilder) jwtForPolicy(krtctx krt.HandlerContext, policy v1alpha1.TrafficPolicy, out *trafficPolicySpecIr) error {
	spec := policy.Spec
	if spec.JWT == nil {
		return nil
	}
	jwtIr, err := convertJwtValidationConfig(krtctx, policy.Name, policy.Namespace, spec.JWT, b.commoncol.Secrets)
	if err != nil {
		return err
	}
	out.jwt = jwtIr

...
func ProcessAIBackendForAgentGateway(ctx krt.HandlerContext, be *v1alpha1.Backend, secrets krt.Collection[*corev1.Secret]) ([]*api.Backend, error) {
	...
	if be.Spec.AI.LLM != nil {
		aiBackend = buildAIBackendFromLLM(ctx, beName, be.Spec.AI.LLM, secrets, be.Namespace)
		...
		aiBackend = buildAIBackendFromLLM(ctx, beName, &be.Spec.AI.MultiPool.Priorities[0].Pool[0], secrets, be.Namespace)
	} else {
...
func buildAIBackendFromLLM(ctx krt.HandlerContext, beName string, llm *v1alpha1.LLMProvider, secrets krt.Collection[*corev1.Secret], namespace string) *api.Backend {

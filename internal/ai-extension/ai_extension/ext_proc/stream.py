...
class Info:
    ...
    def set_headers(
        self,
        header_rules: Iterable[ai_pb2.AIPromptGuard.Webhook.HeaderMatch],
        ...
        for header in headers.headers.headers:
            ...
            for rule in header_rules:
                match rule.match_type:
                    case ai_pb2.AIPromptGuard.Webhook.HeaderMatch.EXACT:
                        if header.key == rule.key:
                            self.headers[header.key] = header.raw_value.decode("utf-8")
                    case ai_pb2.AIPromptGuard.Webhook.HeaderMatch.PREFIX:
                        if header.key.startswith(rule.key):
                            self.headers[header.key] = header.raw_value.decode("utf-8")
                    case ai_pb2.AIPromptGuard.Webhook.HeaderMatch.SUFFIX:
                        if header.key.endswith(rule.key):
                            self.headers[header.key] = header.raw_value.decode("utf-8")
                    case ai_pb2.AIPromptGuard.Webhook.HeaderMatch.CONTAINS:
                        if rule.key in header.key:
                            self.headers[header.key] = header.raw_value.decode("utf-8")
                    case ai_pb2.AIPromptGuard.Webhook.HeaderMatch.REGEX:
...
class Handler:
    ...
    provider: Provider
    llm_provider: str
    req_webhook: ai_pb2.AIPromptGuard.Webhook | None = None
    resp_webhook: ai_pb2.AIPromptGuard.Webhook | None = None
    req_regex: list[EntityRecognizer] | None = None
    req_regex_action: ai_pb2.AIPromptGuard.Regex.Action = (
        ai_pb2.AIPromptGuard.Regex.Action.MASK
    )
    req_moderation: tuple[AsyncModerations, str] | None = None
    req_custom_response: ai_pb2.AIPromptGuard.Request.CustomResponse | None = None
    ...
    async def update_cache(
        ...
        """Checks if the cache is enabled and performs an update."""

        if self.resp.is_streaming and not self.stream_chunks.is_completed:
            # This means even we got end of stream from envoy but there might
            # be network issue and the stream is incomplete, so skip caching
            return

        # TODO(andy): check if response status code is 200?

...
def init_presidio_config(
    guardrails_regex: ai_pb2.AIPromptGuard.Regex,
) -> list[EntityRecognizer]:
    recognizers: list[EntityRecognizer] = []
    compiled_regex: list[Pattern] = []
    for builtin in guardrails_regex.builtins:
        match builtin:
            case ai_pb2.AIPromptGuard.Regex.BuiltIn.CREDIT_CARD:
                recognizers.append(CreditCardRecognizer())
            case ai_pb2.AIPromptGuard.Regex.BuiltIn.SSN:
                recognizers.append(UsSsnRecognizer())
            case ai_pb2.AIPromptGuard.Regex.BuiltIn.PHONE_NUMBER:
                recognizers.append(PhoneRecognizer())
            case ai_pb2.AIPromptGuard.Regex.BuiltIn.EMAIL:
